...
import FormLayoutsBasic2 from '@views/forms/form-layouts/FormLayoutsBasic2'
...
const UserListTable = ({ tableData }) => {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('entry-date', {
        header: 'Entry At',
        cell: ({ row }) => <Typography>{row.original.ENT_DT?.slice(0, 10)}</Typography>
      }),
...
const handleGenerateReport = async () => {
  setLoading(true)
  if (user) {
    const requestUrl =  `${dbUrl}/cu-customer-details?username=${user.number}`
    try {
  ...
  return (
    <>
      ...
      >
        ...
        <CardHeader title='Client Data' />
        {data && data.length > 0 &&
 <FormLayoutsBasic2 data={data}/>} 
